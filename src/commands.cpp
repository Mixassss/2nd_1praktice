...
void BaseData::isValidSelect(string& command) { 
    Filters conditions;
    Hash_table<string, Filters> cond;
        ...
        if (command.find_first_of("FROM") != -1) {
            ...
            while (command.substr(0, 4) != "FROM") {
                ...
                for (i = 0; i < tablesname.size(); ++i) { // проверка, сущ. ли такая таблица
                    if (conditions.table == tablesname.getElementAt(i)) {
                        check = true;
                        break;
                    }
                }
                ...
                check = false;
                stringstream iss(coloumnHash.getValueAt(i));
                ...
                cond.insert(conditions.table, conditions);
            }
            ...
            while (!command.empty()) { // пока строка не пуста
                ...
                if (iter + 1 > cond.size() || token != cond.getKeyAt(iter)) {
                ...
                if (command.find_first_of(' ') != -1) {
                    ...
                    if (position != -1) {
                        ...
                        if (token.find_first_of('.') != -1) {
                            ...
                            string table, column;
                            ss >> table >> column;
                            if (table == cond.getKeyAt(0)) { // проверка таблицы в where
                                ...
                                if ((position != -1) && (command[0] == '=')) {
                                    ...
                                    if (position == -1) { // если нет лог. операторов
                                        if (command.find_first_of('.') == -1) { // если просто значение
                                            ...
                                            selectWithValue(cond, table, conditions);
                                        } else { // если столбец
                                        ...
                                        if ((position != -1) && (command.substr(0, 2) == "or" || command.substr(0, 3) == "and")) {
                                            ...
                                            if (position != -1) {
                                                ...
                                                if (token.find_first_of('.') != -1) {
                                                    ...
                                                    Hash_table<string, string> tables;
                                                    Hash_table<string, string> columns;
                                                    tables.insert(table, table);
                                                    columns.insert(column, column);
...
void BaseData::sample(Hash_table<string, int>& stlbindex, Hash_table<string, string>& tables) {
    for (int i = 0; i < tables.size(); ++i) {
        ...
        stringstream tableStream(tableData);
        string row;
        cout << "Результаты выборки:" << endl;
        while (getline(tableStream, row)) {
            stringstream rowStream(row);
            string token;
            int currentIndex = 0;
            while (getline(rowStream, token, ',')) {
                ...
                if (currentIndex == columnIndex) {
                    cout << token << endl; // Вывод значения столбца
                    break;
                }
                currentIndex++;
            }
